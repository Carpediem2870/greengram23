...
public class UserService {
    ...
    public ResVo postUserSignup(UserSignupDto dto) {
        ...
        UserSignupProcDto pDto= UserSignupProcDto.builder().
                uid(dto.getUid()).upw(hashedPw).nm(dto.getNm()).pic(dto.getPic()).build();
        int affectedRows= mapper.postUserSignup(pDto);
        if(affectedRows==0) {return new ResVo(0);}
        return new ResVo(pDto.getIuser());
    }

    public UserInfoVo getUserInfo(int targetIuser) {
        return mapper.getUserInfo(targetIuser);
    }


    public ResVo updUserInfo(UserPatchPicDto dto){

        return new ResVo(mapper.updUserInfo(dto));
    }
}
